...
subroutine buffer_v
  ...
  if (head) print*, '  check PID range after buffer_v:',&
            minval(pid(:sum(rhoc*int(1,kind=8)))),maxval(pid(:sum(rhoc*int(1,kind=8))))
  call toc(6)
endsubroutine
...
subroutine buffer_xvp(dox,dov)
  ...
  do itx=2,nnt ! skip tile_x=1
    !$omp paralleldo default(shared) schedule(dynamic) private(iz,iy)
    do iz=1,nt
    do iy=1,nt
      if (dox) &
      xp(:,idx_b_l(iy,iz,itx,ity,itz)+1:ppl0(iy,iz,itx,ity,itz))=&
  ...
  do itx=1,nnt
    !$omp paralleldo default(shared) schedule(dynamic) private(iz)
    do iz=1,nt
      if (dox) &
      xp(:,idx_b_l(1-ncb,iz,itx,ity,itz)+1:idx_b_r(0,iz,itx,ity,itz))=&
  ...
  !$omp paralleldo default(shared) schedule(dynamic) private(ity,itx)
  do ity=1,nnt
  do itx=1,nnt
    if (dox) &
    xp(:,idx_b_l(1-ncb,1-ncb,itx,ity,itz)+1:idx_b_r(nt+ncb,0,itx,ity,itz))=&
