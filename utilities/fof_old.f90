...
program CUBE_FoF
  ...
  integer,parameter :: nfof=ng_global ! nfof is the resolution to do percolation
  ...
  real,parameter :: np_halo_min=20 ! minimum number of particles to be a halo
  ...
  integer(4) cur_checkpoint,i,j,k,l,itx,ity,itz,np,nlist,idx(3),n_friend,ngroup,ihalo,nhalo
  integer(4) iq1,iq2,iq3,np_candidate,np_central,pbc(3),jq(3),is1,is2
  real pos1(3),rp,rp2,rsq,xf_hoc(3),vreal(3),torque(3,3),tide(6),jeig(3),t_opt(3),a_opt,r_equiv,msim2phys
  real mu_qj,mu_uj,mu_xj,mu_vj,mu_qu,mu_xv,sj(3),qq_halo(3,3),tide_halo(3,3),dd(3,3,3),qq(3,3,3)
  integer(4),allocatable :: rhoc(:,:,:,:,:,:)
  real(4),allocatable :: vc(:,:,:,:,:,:,:),phi(:,:,:)[:,:,:]
  integer(izipx),allocatable :: xp(:,:)
  integer(izipv),allocatable :: vp(:,:)
  integer(izipi),allocatable :: pid(:),iph_halo_all(:),iph_halo(:),hoc(:,:,:),ll(:),ip_party(:),ip_friend(:),llgp(:),hcgp(:),ecgp(:),isort_mass(:)
  real,allocatable :: xf(:,:),vf(:,:),xf_party(:,:),dx(:,:),dv(:,:),dq(:,:),du(:,:),pos_fof(:,:),mu_quj(:),mu_xvj(:)
  real,allocatable :: x_mean_all(:,:),x_mean(:,:),v_mean_all(:,:),v_mean(:,:),q_mean_all(:,:),q_mean(:,:),u_mean_all(:,:),u_mean(:,:),mu(:,:)
  real,parameter :: lsim2pc=1e6 * box/ng_global/h0
  real,parameter :: G_phys=0.0043 ! pc * M_solar^-1 * (km/s)^2
  real,parameter :: rho_crit=2.7756e11 ! M_solar Mpc^-3 h^-2
  real,parameter :: grid_per_p=(ratio_cs/np_nc)**3 * merge(2,1,body_centered_cubic)
  ...
  if (head) then
    ...
    print*,'ng,nfof=',int(ng,2),int(nfof,2)
  ...
  sync all; n_checkpoint=n_checkpoint[1]; z_checkpoint(:)=z_checkpoint(:)[1]
  ...
  do cur_checkpoint=n_checkpoint,2,-1
    ...
    allocate(xf(3,nplocal),vf(3,nplocal),xf_party(3,nlist),pid(nplocal),xp(3,nplocal),rhoc(nt,nt,nt,nnt,nnt,nnt)) ! traditional checkpoint arrays
    open(11,file=output_name('vp'),access='stream'); read(11) vf; close(11)
    open(11,file=output_name('id'),access='stream'); read(11) pid; close(11)

    ! allocate(xp(3,nplocal),vp(3,nplocal),rhoc(nt,nt,nt,nnt,nnt,nnt),vc(3,nt,nt,nt,nnt,nnt,nnt))
    ...
    do iq1=1,nfof
      ...
      do while (ip/=0)
        ...
        ip_party(np_central)=ip ! particle index list in the party
    ...
    allocate(phi(0:ng+1,0:ng+1,0:ng+1)[nn,nn,*])
    open(11,file=output_name('phi1'),status='old',action='read',access='stream')
    read(11) phi(1:ng,1:ng,1:ng)
    close(11)
    sim%cur_checkpoint=cur_checkpoint
    print*, '  buffer phi'
    phi(0,:,:)=phi(ng,:,:)[inx,icy,icz]
    phi(ng+1,:,:)=phi(1,:,:)[ipx,icy,icz]
    sync all
    phi(:,0,:)=phi(:,ng,:)[icx,iny,icz]
    phi(:,ng+1,:)=phi(:,1,:)[icx,ipy,icz]
    sync all
    phi(:,:,0)=phi(:,:,ng)[icx,icy,inz]
    phi(:,:,ng+1)=phi(:,:,1)[icx,icy,ipz]
    ...
    do ip=1,nplocal
      if (hcgp(ip)/=ip .and. hcgp(ip)/=0) then
        ...
        do while (jp/=0) ! loop over group members
          ...
          dv(:,np)=vf(:,jp)
          ! initial position
          qi=qgrid(pid(jp)) ! Lagrangian grid number ! \in {1,2,...,ng_global}
          dq(:,np)=modulo(qi-qi_hoc+ng_global/2,ng_global)-ng_global/2 ! qpos wrt qhoc
          ! initial velocity
        ...
        q_mean_all(:,nhalo)=modulo(qi_hoc-0.5+sum(dq(:,:np),2)/real(np),real(ng_global)) ! q center of mass
    ...
    x_mean=x_mean_all(:,:nhalo)*ng_global ! convert into unit of fine cell
    ...
    do ihalo=1,nhalo !! omp parallel do
      ...
      do while (jp/=0) ! loop over halo members
        ...
        qi=qgrid(pid(jp)) ! Lagrangian grid number ! \in {1,2,...,ng_global}
        dq(:,np)=modulo(qi-hcat(ihalo)%q+ng_global/2,ng_global*1.)-ng_global/2 ! in unit of fine grid
        du(1,np)=-phi(qi(1)+1,qi(2),qi(3))+phi(qi(1)-1,qi(2),qi(3))
        du(2,np)=-phi(qi(1),qi(2)+1,qi(3))+phi(qi(1),qi(2)-1,qi(3))
        du(3,np)=-phi(qi(1),qi(2),qi(3)+1)+phi(qi(1),qi(2),qi(3)-1)
      ...
      hcat(ihalo)%lambda_B = (norm2(hcat(ihalo)%je)/hcat(ihalo)%hmass * (sim%vsim2phys*lsim2pc*ng_global)) &
    ...
    if (write_particles) then
      open(21,file=output_name('FoF_particles_b'//b_link_string),status='replace',access='stream')
      do ihalo=1,nhalo
        jp=iph_halo(ihalo)
        do while (jp/=0)
    ...
    deallocate(iph_halo,xf,vf,xf_party,hoc,ll,ip_party,ip_friend,llgp,hcgp,ecgp,pos_fof,pid)
    deallocate(dx,dv,dq,du,mu_quj,mu_xvj,hcat)
    call system_clock(t2,t_rate)
    print*, 'total time =',real(t2-t1)/t_rate,'secs';
  enddo
  print*,'CUBE_FoF done'
  ...
  contains
    ...
    function qgrid(pid0)
      implicit none
      integer(8) pid0,pidtemp,n2,qgrid(3)
      n2=ng_global**2
      pidtemp=pid0-1
      qgrid(3)=1+pidtemp/n2
      qgrid(2)=1+(pidtemp-(pidtemp/n2)*n2)/ng_global
      qgrid(1)=1+modulo(pidtemp,ng_global)
    endfunction
